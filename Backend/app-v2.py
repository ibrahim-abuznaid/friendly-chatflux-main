...
def process_query(user_query, qtt="basic", llm_model="openai"):
    """Complete RAG pipeline with specified query translation technique and LLM model"""
    try:
        ...
        print(f"Loading documents from: {docs_path}")  # Debug print
        ...
        try:
            ...
            print(f"Created {len(document_splits)} document splits")  # Debug print
        except Exception as e:
            raise Exception(f"Error preparing documents: {str(e)}")
        ...
        if qtt == "basic":
            ...
            return {
                ...
                "response": create_rag_chain(vectorstore).invoke(user_query).content,
...
def send_message():
    try:
        ...
        qtt = data.get("qtt", "basic")  # Get QTT from request, default to basic
        llm_model = data.get("llm_model", "openai")  # Get LLM model from request, default to openai
        
        if not message:
            return jsonify({"error": "No message provided"}), 400
            
        result = process_query(message, qtt, llm_model)
        ...
        return jsonify({
            ...
            "context": {
                ...
                "llm_model": llm_model,  # Add model info to response
                **({"generated_queries": result["generated_queries"]} if qtt == "fusion" else {}),
                **({"decomposed_query": result["decomposed_query"]} if qtt == "decomposition" else {}),
                **({"hypothetical_doc": result["hypothetical_doc"]} if qtt == "hyde" else {})
            }
        })
